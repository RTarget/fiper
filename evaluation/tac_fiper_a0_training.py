...
def run_cpu_smoke(source, output_dir, provenance):
    ...
    model = ObservationActionTemporalModel(**config).cpu()
    ...
    model.eval()
    restored = ObservationActionTemporalModel(**saved["model_config"]).cpu().eval()
    ...
    manifest = dict(
        status="PASS", scope="sorting_development_smoke_only", device="cpu", seed=seed,
    ...
    print(f"CPU TRAINING PASS: {output.resolve()}")
    return manifest
